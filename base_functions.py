...
def file_to_set(file_name):
    if not os.path.isfile(file_name):
        ...
        try:
            ...
            print(Fore.GREEN + f"[-BF Error]: Created file {fileName}. Please fill this file with your probable targets or just run the spider on the website!" + Style.RESET_ALL)
...
def set_to_file(links, file_name):
    if not os.path.isfile(file_name):
        print(Fore.LIGHTYELLOW_EX + "[-BF Error]: File not found. Dont worry it will auto-create but you need to fill it with urls or run the SPIDER!" + Style.RESET_ALL)
        write_file(file_name, '')
